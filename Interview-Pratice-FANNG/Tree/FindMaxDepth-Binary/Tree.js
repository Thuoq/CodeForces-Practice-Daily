...
class BinarySearchTree {
    ...
    traverseInOrderRNL(node, list, currentLevel) {
        if(node.right) {
  
            this.traverseInOrderRNL(node.right, list, currentLevel+1)
        }
        if(currentLevel >= list.length) { 
            // 0 length 0 , 1  >= 1 check
            list.push({value: node.value,currentLevel})
        }
        if(node.left) {
            
            this.traverseInOrderRNL(node.left, list, currentLevel+1)
        }
        return list
    }
    DFSInOrderRNL() {
        return this.traverseInOrderRNL(this.root,[],0)
    }
}
...
console.log(tree.DFSInOrderRNL())
// //     9
// //  4     20
// //1  6  15  170
